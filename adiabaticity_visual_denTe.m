...
for idiag = start_diag: end_diag
	...
	den_pert_norm = zonal_pert(denp(nr_diag, :, :)) ./ denp(nr_diag, :, :);
	phi_pert_norm = zonal_pert(phip(nr_diag, :, :)) ./ Tep(nr_diag, :, :);
	Te_pert_norm = zonal_pert(Tep(nr_diag, :, :)) ./ Tep(nr_diag, : ,:);
	fig = figure;
	...
	fig_name = fullfile('figs', 'adiabaticity_diag', ...
		['r=', num2str(r_diag), 'cm_den_phi_Te_pert', sprintf('%4.4d', idiag), '.png']);
