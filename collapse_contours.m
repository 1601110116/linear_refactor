...
%---input---
start_diag = 1537; %1440;%1241; %1135;  %499 818
...
load(fullfile('data', ['dat', sprintf('%4.4d', start_diag)]), 'den', 'phi', 'w')
tmp = floor(nx/2);
...
for idiag = start_diag: end_diag
   ...
   set(gca, 'yticklabel', []);
   x_lim = get(gca, 'XLim');
   text(x_lim(1), x_lim(end), ['\fontsize{', num2str(no_size), '}', num2str(igca)], ...
...
print(gcf, '-dpng', fullfile('figs', ['collapse_from_t=', sprintf('%4.4d', start_diag), '.png']));
print(gcf, '-depsc', fullfile('figs', ['collapse_from_t=', sprintf('%4.4d', start_diag), '.eps']));
