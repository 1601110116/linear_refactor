...
for idiag = start_diag: end_diag
	...
	n0 = squeeze(zonal_average(denp));
	inv_Ln = -1/(2*dr) * (n0(nr_start+1: nr_end+1) - n0(nr_start-1: nr_end-1)) ...
		./ n0(nr_start: nr_end);
	Bouss = inv_Ln .* squeeze(zonal_average(vEthtp(nr_start: nr_end, :, :)));
	Bouss_t(:, idiag-start_diag+1) = Bouss;
...
dtBouss = 1/(2*dt*nt_per_diagnose) ...
	* (Bouss_t(3:end-2, 3:end) - Bouss_t(3:end-2, 1:end-2));
...
subplot(2,1,2);
s3 = surf(1e3*tX(2:end-1), r_axis, normalize_factor*perp_drive_ex_t(:, 2:end-1));  hold on;
s4 = surf(1e3*tX(2:end-1), r_axis, normalize_factor*para_drive_ex_t(:, 2:end-1));
%s5 = surf(1e3*tX(2:end-1), r_axis(3:end-2), normalize_factor*dtS_minus_sum);
s5 = surf(1e3*tX(2:end-1), r_axis(3:end-2), normalize_factor*dtS);
s6 = surf(1e3*tX(2:end-1), r_axis(3:end-2), normalize_factor*dtBouss);
...
set(s6, 'FaceAlpha', 0.3, 'EdgeColor', 'y', 'FaceColor', 'y');
xlabel('t/ms');
ylabel('r/cm');
...
lgd = legend('$$F_{\perp}^{\dagger}$$', '$$F_{\parallel}^{\dagger}$$', ...
	'$$\rm{d}_{t}S^{\dagger}$$', '$$1/L_n d_t \left<v_{E,\theta}\right>$$');
%lgd = legend('$$F_{\perp}^{\dagger}$$', '$$F_{\parallel}^{\dagger}$$');

set(lgd, 'interpreter', 'latex');
set(lgd, 'FontSize', legend_font_size);

png_file = fullfile('figs', ['t=', num2str(start_diag), '-', num2str(end_diag), 'Bouss.png']);
eps_file = fullfile('figs', ['t=', num2str(start_diag), '-', num2str(end_diag), 'Bouss.eps']);
