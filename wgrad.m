...
r_end = 9.5;
font_size = 13;
fig_position = [50, 50, 700, 1200];  % [left, bottom, width, height]
line_width = 1;
...
w_max_t = ZF_rdiag_t;
grad_w = zeros(nx+2, nx+2, nz+2);
ZF_curv_max_t = ZF_rdiag_t;
grad_w_max_t = ZF_rdiag_t;
...
for idiag = start_diag: end_diag
	...
	grad_w(2:end-1, 2:end-1, :) = ...
		(1/(2*dx) * (w(3:end, 2:end-1, :) - w(1:end-2, 2:end-1, :))).^2 + ...
		(1/(2*dx) * (w(2:end-1, 3:end, :) - w(2:end-1, 1:end-2, :))).^2;
	grad_w = sqrt(grad_w);
	grad_w_max_t(idiag-start_diag+1) = squeeze(max(max(max(grad_w, [], 3), [], 2), [], 1));
	ZF_curv = 1/(2*dr) * (ZF_vorticity(3:end) - ZF_vorticity(1:end-2));
	ZF_curv_max_t(idiag-start_diag+1) = max(ZF_curv);
end
%%
close;
fig = figure;
...
subplot(4,1,4);
plot(tX_ms, cs0/rhos0^2*abs(ZF_curv_max_t), 'b-', 'LineWidth', line_width);  hold on;
plot(tX_ms, cs0/rhos0^2*grad_w_max_t, 'g-', 'LineWidth', line_width);  hold off;
set(gca, 'XTick', x_tick);
set(gca, 'XLim', x_lim);

set(gca, 'fontSize', font_size);
lgd4 = legend( ...
	'$$\rm Max\left\{\left|\it \partial_{r}\left(\frac{1}{r}\it\partial_{r}\left<rv_{E,\theta}\right>\right)\right|\right\}$$', ...
	'$$\rm Max\left\{\left|\it \nabla w\right|\right\}$$');
set(lgd4, 'interpreter', 'latex');
set(lgd4, 'FontSize', legend_font_size);
set(lgd4, 'Location', 'NorthWest');
set(lgd4, 'NumColumns', 2);
set(lgd4, 'Box', 'off');


set(fig, 'Position', fig_position);
png_file = fullfile('figs', 'wgrad.png');
eps_file = fullfile('figs', 'wgrad.eps');
