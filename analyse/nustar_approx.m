...
%%
%---input---
start_diag = 667;  % the figure starts from start_diag+1 and ends at end_diag-1
end_diag = 709;  
r_start = 0.4;  % cm
r_end = 6;  % cm
font_size = 20;
legend_font_size = 24;
no_size = 26;
fig_size = [50, 50, 1500, 700]; %[left bottom width height]
view_angle = [60, 15];  % [horizontal rotation, vertical elevation]
%-----------
...
txt = text(xlim(1), ylim(2), zlim(2), '(a)') 
...
tmp = get(lgd, 'Position');
tmp(1) = lgd_left;
%set(lgd, 'Position', tmp);
%fig_name = ['t=', num2str(start_diag), '-', num2str(end_diag), 'shear_den_drive_t'];
