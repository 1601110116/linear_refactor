...
start_diag = 514;  % 514; %506
end_diag = 517;
nz_per_slice = 3;  % draw a slice every nz_per_diag x-y planes;

fig_position = [50, 50, 800, 1500];  % [left, bottom, width, height];
view_angle = [18, 30];  % [horizontal rotation, vertical elevation]
font_size = 20;
no_size = 25;
title_size = 27;
line_width = 3;
...
for idiag = start_diag: end_diag
	...
	if save_eps == 1
		print(gcf, '-depsc', ['slices_t=', sprintf('%4.4d', idiag), '.eps']);
		if exist(dst_path, 'dir')
			copyfile(['slices_t=', sprintf('%4.4d', idiag), '.eps'], dst_path);
		else
			disp('Destination path do not exist');
		end
	elseif save_eps == 0
		print(gcf, '-dpng', ['3D_structures/3D_t=', sprintf('%4.4d', idiag), '.png']);
	else
