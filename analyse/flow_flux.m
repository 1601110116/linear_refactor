...
%%
%---input---
start_diag = 1213;  % the figure starts from start_diag+1 and ends at end_diag-1
end_diag = 1239; %last_diag;  
r_start = 0.4;  % cm
r_end = 6;  % cm
font_size = 13;
legend_font_size = 14;
no_size = 16;
fig_size = [50, 50, 900, 700]; %[left bottom width height]
...
txt = text(xlim(2), ylim(2), zlim(1), '(a)'); 
...
set(lgd, 'Location', 'NorthWest');
xlim = get(gca, 'XLim');  ylim = get(gca, 'YLim'); zlim = get(gca, 'ZLim');
txt = text(xlim(2), ylim(2), zlim(1), '(b)'); 
set(txt, 'fontSize', no_size);

subplot(2,2,3);
s7 = surf(1e3*tX, r_axis, cs0*ZF_t(:, 2:end-1));
set(s7, 'FaceAlpha', 0.8, 'EdgeColor', 'g', 'FaceColor', 'interp');
xlabel('t/ms');  ylabel('r/cm');
zlabel('$$\left<v_{E,\theta}\right>/\rm \left(cm/s\right)$$', 'interpreter', 'latex');
set(gca, 'XLim', [1e3*tX(1), 1e3*tX(end)]);
set(gca, 'YLim', [r_axis(1), r_axis(end)]);
view(view_angle);
set(gca, 'fontSize', font_size);
xlim = get(gca, 'XLim');  ylim = get(gca, 'YLim'); zlim = get(gca, 'ZLim');
txt = text(xlim(2), ylim(2), zlim(1), '(c)'); 
set(txt, 'fontSize', no_size);


subplot(2,2,4);
s7 = surf(1e3*tX, r_axis, cs0^2*RS_t(:, 2:end-1));
set(s7, 'FaceAlpha', 0.8, 'EdgeColor', 'g', 'FaceColor', 'interp');
xlabel('t/ms');  ylabel('r/cm');
zlabel('$$\left<v_{r}v_{E,\theta}\right>/\rm \left(cm^{2}/s^{2}\right)$$', ...
	'interpreter', 'latex');
set(gca, 'XLim', [1e3*tX(1), 1e3*tX(end)]);
set(gca, 'YLim', [r_axis(1), r_axis(end)]);
view(view_angle);
set(gca, 'fontSize', font_size);
xlim = get(gca, 'XLim');  ylim = get(gca, 'YLim'); zlim = get(gca, 'ZLim');
txt = text(xlim(2), ylim(2), zlim(1), '(d)'); 
set(txt, 'fontSize', no_size);



fig_name = 'acc_den_drive';
