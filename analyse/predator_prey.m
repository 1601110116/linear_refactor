...
no_size = 33;  % font size of the number of each subplot
...
pcolor(tX_ms, rX(nr_start: nr_end), log10(cs0*ZF_t));
...
gca_position = [gca_left, init_position(2), gca_width, gca_height];
set(gca, 'Position', gca_position);
pcolor(tX_ms, rX(nr_start: nr_end), log10(denref^2*m3_autopower_t));
colormap jet;  colorbar;  shading interp;
...
gca_position = [gca_left, init_position(2), gca_width, gca_height];
set(gca, 'Position', gca_position);
pcolor(tX_ms, rX(nr_start: nr_end), rhos0*denref*int_den_t);
colormap jet;  colorbar;  shading interp;
...
pcolor(tX_ms, rX(nr_start: nr_end), lg_int_flux_tX);
colormap jet;  colorbar;  shading interp;
...
%set(gca, 'FontSize', font_size);
%
print(fig, '-dpng', 'pp.png');
print(fig, '-depsc', 'pp.eps');
if exist(dst_path, 'dir')
	copyfile('pp.eps', dst_path);
else
